...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        ...
        <p class="text-gray-600 mt-1">Manage your subscription and payments</p>
    </div>
    ...
    <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg shadow-lg p-6 text-white">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-white bg-opacity-20 rounded-lg p-4">
                ...
                <div class="text-3xl font-bold mt-1">GHC {{ number_format($balanceInfo['total_paid'], 2) }}</div>
            </div>
            <div class="bg-white bg-opacity-20 rounded-lg p-4">
                <div class="text-sm opacity-90">Expected Amount</div>
                <div class="text-3xl font-bold mt-1">GHC {{ number_format($balanceInfo['expected_amount'], 2) }}</div>
            </div>
            <div class="bg-white bg-opacity-20 rounded-lg p-4">
                ...
                <div class="text-3xl font-bold mt-1 {{ $balanceInfo['has_credit'] ? 'text-green-200' : 'text-red-200' }}">
                    GHC {{ number_format(abs($balanceInfo['balance']), 2) }}
                </div>
                @if($balanceInfo['has_credit'])
                    <div class="text-sm mt-1 opacity-90">{{ $balanceInfo['days_remaining'] }} weekday{{ $balanceInfo['days_remaining'] !== 1 ? 's' : '' }} remaining</div>
                @endif
            </div>
        </div>
    </div>
        ...
        <div class="p-6">
                ...
                <div class="flex items-center justify-between flex-wrap gap-4">
                    <div>
                        <div class="flex items-center">
                            ...
                            <span class="text-green-700 font-semibold">Active Subscription</span>
                        </div>
                        <p class="text-green-600 mt-1 ml-7">
                            @if($balanceInfo['has_credit'])
            ...
            <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded mb-4">
                <div class="flex items-center">
                    ...
                    <span class="text-yellow-700 font-semibold">No Active Subscription</span>
                </div>
                <p class="text-yellow-600 mt-1 ml-7">Subscribe now to access premium features</p>
            </div>
            @endif
            ...
            <div class="grid grid-cols-1 gap-6 mt-6">
                ...
                <div class="border-2 border-purple-200 rounded-lg p-6 bg-purple-50">
                    <div class="mb-4">
                        <h3 class="text-xl font-bold text-purple-900 mb-2">Custom Payment</h3>
                        <p class="text-purple-700 text-sm">Pay any amount to add credit to your account (GHC 3 = 1 weekday)</p>
                    </div>
                    <form action="{{ route('payment.initialize') }}" method="POST" class="space-y-4" x-data="{ amount: '' }">
                        @csrf
                        <div>
                            <label for="custom_amount" class="block text-sm font-medium text-gray-700 mb-2">Enter Amount (GHC)</label>
                            <input 
                                type="number" 
                                name="custom_amount" 
                                id="custom_amount"
                                ...
                                required>
                            <p class="text-sm text-gray-600 mt-2" x-show="amount > 0" x-cloak>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="border-2 border-green-200 rounded-lg p-6 hover:border-green-400 transition">
                            <div class="text-center">
                                <div class="text-4xl font-bold text-green-600 mb-2">GHC 3</div>
                                <div class="text-gray-600 mb-1">Daily Subscription</div>
                                <div class="text-sm text-gray-500 mb-4">Valid for 1 weekday</div>
                        ...
                        <div class="border-2 border-blue-200 rounded-lg p-6 hover:border-blue-400 transition">
                            <div class="text-center">
                                <div class="text-4xl font-bold text-blue-600 mb-2">GHC 60</div>
                                <div class="text-gray-600 mb-1">Monthly Subscription</div>
                                <div class="text-sm text-gray-500 mb-1">Valid for 20 weekdays</div>
                                <div class="inline-block bg-blue-100 text-blue-800 text-xs font-semibold px-2 py-1 rounded mb-3">
                                    Save GHC 30
                                </div>
                                <form action="{{ route('payment.initialize') }}" method="POST">
                                    @csrf
